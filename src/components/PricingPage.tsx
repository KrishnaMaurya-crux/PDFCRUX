...
// Pricing data per region — single source of truth
const PRICING = {
  india: {
    premium: { monthly: "₹249", annual: "₹1,999", monthlyEquiv: "₹166/mo", savePercent: "Save 33%" },
    free: "₹0",
    code: "INR",
    symbol: "₹",
  },
  global: {
    premium: { monthly: "$5.99", annual: "$49", monthlyEquiv: "$4.08/mo", savePercent: "Save 32%" },
    free: "$0",
    code: "USD",
    symbol: "$",
  },
...
const PREMIUM_FEATURES: PricingPlan["features"] = [
...
// Build plan from region pricing data
function getPremiumPlan(region: Region): PricingPlan {
  const p = PRICING[region].premium;
  return {
    name: "Premium",
    monthly: p.monthly,
    annual: p.annual,
    annualMonthly: region === "india" ? "₹166" : "$4.08",
...
// ============================================================
// Region Detection (IP-based via API)
// ============================================================
async function detectRegion(): Promise<Region> {
  try {
    ...
    return data.region === "india" ? "india" : "global";
  } catch {
    return "global";
  }
}
...
function PricingCard({
  ...
  if (isEnterprise) {
    ...
    const pp = pricing.premium;
    price = billing === "monthly" ? pp.monthly : pp.annual;
    period = billing === "monthly" ? "/mo" : "/yr";
    monthlyEquiv = billing === "annual" ? pp.monthlyEquiv : "";
  }
...
export default function PricingPage() {
  ...
  const premiumPlan = getPremiumPlan(region);

  // Auto-detect region from IP on mount
  useEffect(() => {
    let cancelled = false;
    detectRegion().then((r) => {
      if (!cancelled) {
            ...
            <div className="p-3 rounded-lg bg-primary/5 border border-primary/10 mb-4">
              ...
              {billing === "annual" && (
                <p className="text-[10px] text-emerald-600 mt-1">
                  {PRICING[region].premium.monthlyEquiv} — Save {PRICING[region].premium.savePercent.replace("Save ", "")}
                </p>
              )}
            </div>
