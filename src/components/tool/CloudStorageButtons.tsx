...
function GoogleDriveLogo({ size = "w-7 h-7" }: { size?: string }) {
  return (
    <svg
      viewBox="0 0 87.3 78"
...
function DropboxLogo({ size = "w-7 h-7" }: { size?: string }) {
  return (
    <svg
      viewBox="0 0 94 78"
      xmlns="http://www.w3.org/2000/svg"
    ...
    >
      ...
      <path d="m94 28.5-23.5-13.5-23.5 13.5v26l23.5-13.5 23.5-13.5z" fill="#0061FF" opacity=".6" />
      <path d="m47 51.5 23.5 13.5v-26l-23.5 13.5z" fill="#0061FF" opacity=".8" />
    </svg>
  );
}
...
export default function CloudStorageButtons({
  ...
  const gdReady = isGoogleDriveReady;
  const dbReady = isDropboxReady;

  const gdLoading = loadingProvider === "google-drive";
  const dbLoading = loadingProvider === "dropbox";

  // Tooltips always show the action name (never "Not configured")
  ...
  // Don't render anything if neither provider is ready
  if (!gdReady && !dbReady) return null;
    ...
    <div className={`flex items-center gap-4 justify-center ${className ?? ""}`}>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              ...
              className="group relative w-14 h-14 rounded-full bg-white dark:bg-card border-2 border-gray-200 dark:border-gray-600 shadow-md hover:shadow-lg hover:shadow-blue-100 dark:hover:shadow-blue-950/30 transition-all duration-200 hover:scale-110 hover:border-blue-300 dark:hover:border-blue-500 flex items-center justify-center disabled:opacity-60 disabled:cursor-wait disabled:hover:scale-100"
            >
              {gdLoading ? (
          ...
          <TooltipContent side="bottom" sideOffset={6}>
            <p className="font-medium">{gdTooltip}</p>
          </TooltipContent>
        </Tooltip>
      )}
      ...
      {dbReady && (
        <Tooltip>
          ...
          <TooltipContent side="bottom" sideOffset={6}>
            <p className="font-medium">{dbTooltip}</p>
          </TooltipContent>
        </Tooltip>
      )}
    </div>
