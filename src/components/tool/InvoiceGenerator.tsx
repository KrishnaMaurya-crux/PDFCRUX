...
import {
  ...
  ToggleLeft, ToggleRight, Truck, History, Wallet, Crown,
} from "lucide-react";
...
export default function InvoiceGenerator() {
  ...
  // Watermark: default ON (free users). Premium users can disable.
    ...
    <section className="relative pt-24 pb-16 sm:pt-32 sm:pb-20 lg:pt-40 lg:pb-28 hero-gradient overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        >
          ...
          >
            {[
              { icon: CheckCircle2, text: "Free Plan Available", color: "text-emerald-500" },
              ...
              <div className="w-full lg:w-[60%] space-y-3">
                ...
                <div className="flex items-center justify-between p-4 rounded-xl bg-card border shadow-sm">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="space-y-0.5">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-sm">Branding Watermark</span>
                        <Badge className="text-[8px] px-1.5 py-0 bg-amber-100 text-amber-700 border-amber-200 hover:bg-amber-100">PRO</Badge>
                      </div>
                      <p className="text-[10px] text-muted-foreground">
                        {watermarkDisabled ? "Watermark removed — clean invoice" : '"Generated by PdfCrux.com" shown on PDF'}
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={() => setWatermarkDisabled(!watermarkDisabled)}
                    className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${watermarkDisabled ? "bg-emerald-500" : "bg-muted"}`}
                    title={watermarkDisabled ? "Disable watermark removal" : "Remove watermark (Premium)"}
                  >
                    <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${watermarkDisabled ? "translate-x-5" : ""}`} />
                  </button>
                </div>
