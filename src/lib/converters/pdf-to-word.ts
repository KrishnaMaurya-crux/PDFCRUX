...
async function sendBatchToGemini(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (!response.ok) {
        let errorMsg = `Server error (HTTP ${response.status}). Please try again.`;
        try {
        ...
        if (attempt < MAX_RETRIES) {
          console.warn(`Batch ${batchIndex} failed (attempt ${attempt + 1}), retrying: ${errorMsg}`);
          continue;
        }
      ...
      if (!data.success) {
        const msg = data.error || "Gemini AI returned no results.";
        if (attempt < MAX_RETRIES) {
